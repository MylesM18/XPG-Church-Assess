...
## Escalation E1 — OPEN, blocks Task 3 only
...
**Do not start Task 3 until Natalie has ruled.** Tasks 1, 2 and 4 are unaffected except for the `beats` field itself, which Task 1 introduces — if E1 is resolved as (a), delete `beats` from Task 1's interface and the two assertions that cover it.
...
⚠️ **Blocked on Escalation E1.** Do not start until Natalie has ruled on the per-beat budget sentence.
