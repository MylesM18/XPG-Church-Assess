...
Any failing check ⇒ `generateProse` returns `null`. Three checks:
...
2. **Numeric containment.** Allowed multiset = numeric tokens extracted from the **draft** ∪ numeric tokens from the **serialized `Diagnosis` struct**. Extraction: regex over ints and decimals including `%` and thousands separators; normalize by numeric value (`parseFloat` after stripping `%` and `,`), so `45` ≡ `45.0`, `45%` → `45`; membership is by **value, not count**. Rule: every numeric token in the AI prose must be in the allowed set, else fail. (The draft already contains every legitimate number — `primary_score`, evidence-trail `ref value` pairs, blind-spot `belief/evidence/gap`, dispersion `spread`, and the fixed `benchmark_note` copy — because the draft *is* `fallbackProse`'s output; the ∪-struct is belt-and-suspenders.)
...
**Stated limitation (accepted).** Numeric containment is a *containment guard, not a proof*: an invented number that happens to equal an allowed value passes. Mitigated by the system-prompt ban, the reword framing (the model is handed the finished numbers, not asked to compute any), and the fact that dangerous fabrications are stats/percentages that rarely collide with the small integer scores in the allowed set. It is a backstop, not a theorem — documented here so no one mistakes it for one.
