...
1. `alter table public.assessment_runs add column closed_at timestamptz null, add column closed_by uuid null references auth.users;` (FK follows the schema's `granted_by`/`accepted_by` convention — nullable, no `on delete`) — no CHECK change; `status` remains `'in_progress' | 'complete'`. `completed_at` is kept and still set on close so existing readers/tests keep working; `closed_at`/`closed_by` are the audit pair.
2. **`close_run(p_church_id uuid)`** — `security definer`; run resolved first via `current_run(...)` (never an inline status filter — the `tests/coverage/current-run-dedup.test.ts` tripwire pattern; `raise exception 'no run for this church'` if none), then `perform public.require_church_admin(v_run.id)` (the helper takes a **run id** — the `save_report` pattern), `raise exception 'run is already closed'` if `status='complete'`, else `update … set status='complete', completed_at=now(), closed_at=now(), closed_by=auth.uid()`. `grant execute … to authenticated`.
...
4. **`save_diagnosis`** — `create or replace` (return type unchanged): remove the `run is already complete` gate (`20260730000100:130-131`), the status/`completed_at` flip (`:138-140`), and the now-unused `v_status` declaration (select only `id` from `current_run`). Everything else identical.
...
- **Run closed:** one line *"Assessment closed on <date>"* (dateless *"Assessment closed"* when `closed_at` is null — old Generate-path runs) + "Reopen assessment" button, also confirmed: *"Members will be able to change their answers again and reminder emails may resume."* Generate / Regenerate / View diagnosis remain available.
...
- **pgTAP** (authored by the agent, executed only by the owner via `npm run test:db`):
  ...
  - `11_*` (get_run_responses): add "returns rows when run is `complete`". `22_*` (get_completed_run_responses): add "returns rows when run is `in_progress`"; its complete-run seed is kept but back-dated (`created_at` − 1 day) so `current_run()` (earliest run) still resolves to it. `24_outreach_reflection_test.sql` (7)/(8): reflection row + share repointed from a later complete run to the church's current run (`plan(19)` unchanged).
  - New `26_close_run_test.sql`, `27_reopen_run_test.sql` (`23_`/`24_` prefixes already exist): admin gate; non-admin raises; fields set / cleared; `run is already closed` / `run is not closed`; `submit_self_response` refused after close and accepted after reopen; coverage RPCs unaffected by status.
